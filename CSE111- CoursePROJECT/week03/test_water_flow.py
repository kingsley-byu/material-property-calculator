from water_flow import water_column_height,pressure_gain_from_water_height, pressure_loss_from_pipe, \
    pressure_loss_from_fittings, reynolds_number, pressure_loss_from_pipe_reduction, pressure_in_psi
from pytest import approx
import pytest

def test_water_column_height():
    assert water_column_height(0.0, 0.0) == approx(0.000, abs=0.001)
...
def test_pressure_gain_from_water_height():
    ...
    assert pressure_gain_from_water_height(50.0) == approx(0489.450, abs=0.001) 
...
def test_reynolds_number():
   ...
   assert reynolds_number(0.048692, 0.00) == approx(0, abs=1)
   assert reynolds_number(0.048692, 0.00) == approx(0, abs=1)
   assert reynolds_number(0.286870, 0.00) == approx(0, abs=1)
   assert reynolds_number(0.286870, 0.00) == approx(0, abs=1)
...
# the test function test if the value in psi return the correct value equivalent in kilopascal(kpi)
def test_pressure_in_psi():
    assert pressure_in_psi(212.1) == approx(30.8, abs=0.1)
    assert pressure_in_psi(158.7) == approx(23.0, abs=0.1)
    assert pressure_in_psi(107.7) == approx(15.6, abs=0.1)
